...
def cv_page(lang):
    ...
    activities=''.join(f'<li>{E(x[lang])}</li>' for x in D.get('activities',[]))
    project=D['project']
    body=f'''<div class="shell cv-toolbar"><a class="breadcrumb" href="index.html">← {tr('Home','首页',lang)}</a><button class="button button-primary print-button" type="button">{icon('print')}{tr('Print / save PDF','打印 / 另存为 PDF',lang)}</button></div><article class="cv-sheet shell"><header class="cv-header"><p class="eyebrow">CURRICULUM VITAE</p><h1>{E(D['name'][lang])} <span>{tr('魏泽','Ze Wei',lang)}</span></h1><p>{E(D['role'][lang])} · {E(D['school'][lang])} · {E(D['university'][lang])}</p><p><a href="mailto:{E(D['email'])}">{E(D['email'])}</a>{(' · ORCID: '+E(D['orcid'])) if D.get('show_orcid',False) and D.get('orcid') else ''}</p></header><section class="cv-section"><h2>{tr('Research profile','研究简介',lang)}</h2>{about}<ul>{research}</ul></section><section class="cv-section"><h2>{tr('Academic appointment','学术任职',lang)}</h2><p><strong>{E(D['university'][lang])}</strong> · {E(D['role'][lang])}, {E(D['school'][lang])}<br>{tr('Jun. 2026 — Present','2026.06 — 至今',lang)} · {E(D['location'][lang])}</p></section><section class="cv-section"><h2>{tr('Education','教育经历',lang)}</h2><ul>{edus}</ul></section>{''.join(sections)}<p class="publication-note">{tr('* Corresponding author.','* 表示通讯作者。',lang)}</p><section class="cv-section"><h2>{tr('Research project','科研项目',lang)}</h2><p><strong>{E(project['funder'][lang])} · {E(project['number'])}</strong><br>{E(project['title'][lang])}<br>{E(project['role'][lang])}</p></section><section class="cv-section"><h2>{tr('Honors & experience','荣誉与实践',lang)}</h2><ul>{awards}</ul><p>{E(D['award_note'][lang])}</p></section><section class="cv-section"><h2>{tr('Academic service','学术服务',lang)}</h2><ul>{services}</ul></section><section class="cv-section"><h2>{tr('Academic activities','学术活动',lang)}</h2><ul>{activities}</ul></section><section class="cv-section"><h2>{tr('Research methods & tools','研究方法与工具',lang)}</h2><p>{E(D['skills'][lang])}</p></section><p class="cv-update">{tr('Updated: September 2026','更新于 2026 年 9 月',lang)}</p></article>'''
